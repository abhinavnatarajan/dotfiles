...
return {
	generator = function(_, cb)
		local cmake_presets = {}
		if vim.fn.filereadable('CMakeUserPresets.json') == 1 then
	...
	cache_key = function(opts)
		return vim.fs.joinpath(opts.dir, 'CMakeUserPresets.json')
	end,
	condition = {
		callback = function(search)
			if not vim.fn.filereadable(vim.fs.joinpath(search.dir, 'CMakeUserPresets.json')) then
				return false, "CMakeUserPresets.json not found"
			end
